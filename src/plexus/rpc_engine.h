...
public:
            ...
            RpcResponseFailure failure = nullptr, int timeout = 15)
  {
    ...
    create(destination, method, buffer,
  ...
  void registerMethod(const std::string &method, std::function<ResponseType(const RequestType&, const RoutedMessage&)> impl)
  {
    ...
    m_methods[method] = [impl](const RoutedMessage &msg, const Protocol::RpcRequest &request,
...
protected:
   ...
   * @param timeout Timeout (in seconds)
   */
  RpcCallPtr create(const NodeIdentifier &destination, const std::string &method,
